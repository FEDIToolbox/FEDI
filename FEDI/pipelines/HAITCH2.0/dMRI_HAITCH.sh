...
##  This version uses the new fedi_* command-line tools                  ##
##                                                                      ##
##########################################################################
...
if [[ ${FEDI_DMRI_PIPELINE_STEPS["STEP8_3DSHORE_RECONSTRUCTION"]}  == "TODO" ]] && [[ ! -e ${LOCKED_STEPS_DIR}/lock_STEP8_3DSHORE_RECONSTRUCTION || ! $NOLOCKS = 1 ]]; then
    ...
    echo "=================================================================================================================="
    echo "Running Motion Correction Pipeline using fedi_dmri_moco"
    echo "=================================================================================================================="
    
    # Use the new fedi_dmri_moco command which handles all iterations internally
    # This replaces the manual iteration loop with a single command
    # Note: fedi_dmri_moco outputs all files (spred, weights, etc.) to the output directory
    # The slice weights will be in the output_dir, not in a separate SLICEWEIGHTS_DIR
    fedi_dmri_moco \
        -d "${WORKING_DMRI}" \
        -a "${BVALSTE}" \
        -e "${BVECSTE}" \
    ...
    # After fedi_dmri_moco completes, the final outputs are in ${MOTIONCORREC_DIR}:
    # - spred5.nii.gz (final SHORE prediction after 6 epochs: iterations 0-5)
    # - working_updated3.nii.gz (final motion-corrected dMRI after 4 registration iterations: 0-3)
    # - rotated_bvecs3 (final rotated b-vectors)
    # - fsliceweights_*.txt (weighting files for each iteration)
    # - registration_iter*/ (registration transformation files)
    
    # Update variables to match expected output names for downstream steps
    # The final spred is spred5 (for 6 epochs: 0-5)
    # The final working_dmri is working_updated3 (for 4 registration iterations: 0-3)
    WORKING_DMRI="${MOTIONCORREC_DIR}/working_updated3.nii.gz"
    BVECSTE="${MOTIONCORREC_DIR}/rotated_bvecs3"
    
    # Copy slice weights to SLICEWEIGHTS_DIR for compatibility with downstream steps if needed
    # (fedi_dmri_moco already creates them in MOTIONCORREC_DIR, but some steps may expect them in SLICEWEIGHTS_DIR)
    if [ -d "${SLICEWEIGHTS_DIR}" ]; then
        cp -f ${MOTIONCORREC_DIR}/fsliceweights_*.txt ${SLICEWEIGHTS_DIR}/ 2>/dev/null || true
        cp -f ${MOTIONCORREC_DIR}/fvoxelweights_*.nii.gz ${SLICEWEIGHTS_DIR}/ 2>/dev/null || true
    fi
